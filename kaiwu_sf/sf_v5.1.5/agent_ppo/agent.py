...
class Agent(BaseAgent):
    ...
    def reset(self, observation):
        ...
        self.feature_process = FeatureProcess(self.hero_camp)
        # 初始化 obs_data/act_data 避免框架在 predict/exploit 异常后访问时报 AttributeError
        self.obs_data = None
        self.act_data = None
    ...
    def observation_process(self, observation):
        feature = self.feature_process.process_feature(observation)
    ...
    def _sample_masked_action(self, logits, legal_action):
        ...
        # target 头：legal_action 格式为 [12*9=108]，需按 main_action 筛选为 9 维
        index = len(self.label_size_list) - 1
        target_legal_action_all = np.reshape(
            legal_actions[index],
            [self.legal_action_size[0], self.legal_action_size[-1] // self.legal_action_size[0]],
        )
        target_legal_action = target_legal_action_all[action_list[0]]
        probs = self._legal_soft_max(logits_split[-1], target_legal_action)
        prob_list += list(probs)
        sample_action = self._legal_sample(probs, use_max=False)
        action_list.append(sample_action)

        target_legal_action_d = target_legal_action_all[d_action_list[0]]
        d_probs = self._legal_soft_max(logits_split[-1], target_legal_action_d)
        d_prob_list += list(d_probs)
        d_action = self._legal_sample(d_probs, use_max=True)
        d_action_list.append(d_action)
